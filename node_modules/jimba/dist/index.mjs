...
var opt = {
  ...
  // enable log output
  _Ob: 0,
  // extra trace for fails
  _F: 0,
  // enable function timing output
  _Tc: 1,
  // counter
  _tNo: 20,
  _Min: -100,
  _Max: 100,
  _FUNCTIONS: /* @__PURE__ */ Object.create(null)
...
function getCallSite() {
  ...
  for (const l of lines) {
    if (!l.startsWith("at "))
      continue;
    if (l.includes("jimba.js"))
      continue;
    if (l.includes("getCallSite"))
      continue;
    return l;
  }
...
function shortSite(siteLine) {
  ...
  if (!m)
    return s.replace(/^at\s+/, "");
  const file = m[1].split("/").pop();
  return `${file}:${m[2]}:${m[3]}`;
}
function parseSite(siteLine) {
  const s = String(siteLine || "");
  const m = s.match(/at\s+(.+?)\s+\((.+):(\d+):(\d+)\)/) || s.match(/at\s+(.+):(\d+):(\d+)/);
  if (!m)
    return {
      fn: "",
      loc: ""
...
var INFO_BADGE_ = "background:#f59e0b;color:#111827;font-weight:700;padding:2px 8px;border-radius:10px";
var INFO_BADGE_BAD = "background:#8b0000;color:#fff;font-weight:700;padding:2px 8px;border-radius:10px";
...
function infoOneLine(obj, siteLine) {
  ...
  const bad = hasBadValue(obj, {
    includeZero: false,
    // set true if you want 0 to count as bad
    maxDepth: 6
  });
...
var ComparisonMethods = class {
  ...
  between(start, end, k = 0) {
    this.actual >= start && this.actual <= end ? pass(this.actual, this.expectedValue, k, this.ctx) : fail(this.actual, this.expectedValue, k, this.ctx);
  }
  /** @param {any} expected @param {number} [k=0] @returns {void} */
  eq(expected, k = 0) {
    expected === this.actual ? pass(this.actual, String(expected), k, this.ctx) : fail(this.actual, String(expected), k, this.ctx);
  }
  ...
  range(min = 0, max = 100, k = 0) {
    typeof this.actual === "number" && this.actual >= min && this.actual <= max ? pass(this.actual, "range pass", k, this.ctx) : fail(this.actual, "range fail", k, this.ctx);
  }
  /** @param {number} [k=0] @returns {void} */
  string(k = 0) {
...
function passes(k = 0) {
  ...
  if (opt._T === 0)
    return;
  if (opt.TOTAL_PASS > 0)
    _console.log("%cTOTAL_PASSES : " + opt.TOTAL_PASS, CSS.badgeTotalPass);
}
function fails(k = 0) {
  if (!(opt._O || opt._T === 1 || opt._R || k !== 0))
    return;
  if (opt.TOTAL_FAIL > 0)
    _console.log("%cTOTAL_ERRORS : " + opt.TOTAL_FAIL, CSS.badgeTotalFail);
}
function funcs(k = 0) {
  if (!(opt._F === 1 || opt._R === 1 || k !== 0))
    return;
...
function tests(k = 0) {
  if (!(opt._T === 1 || opt._R || k !== 0))
    return;
  if (opt.TOTAL_TESTS_PASS > 0)
    _console.log("%cTOTAL_TESTS_PASS : " + opt.TOTAL_TESTS_PASS, "background-color:blue;color:#fff;");
  if (opt.TOTAL_TESTS_FAIL > 0)
    _console.log("%cTOTAL_TESTS_FAIL : " + opt.TOTAL_TESTS_FAIL, CSS.badgeFail);
  if (opt.TOTAL_TESTS_RUN > 0)
    _console.log("%cTOTAL_TESTS_RUN : " + opt.TOTAL_TESTS_RUN, "background-color:blue;color:#fff;");
  if (opt.TOTAL_WRONG_DATA_TYPES_PARAMS > 0)
    _console.log("%cTOTAL_WRONG_DATA_TYPES_PARAMS : " + opt.TOTAL_WRONG_DATA_TYPES_PARAMS, "background-color:blue;color:#fff;");
}
var _timers = /* @__PURE__ */ Object.create(null);
...
var j = {
  ...
  check(title, varString, expectedAnswer, k = 0) {
    ...
    const ok = bothArrays ? compareArrays(varString, expectedAnswer) : bothObjects ? compareObjects(varString, expectedAnswer) : varString === expectedAnswer;
    if (ok) {
      if (opt._FailsOnly === 0) {
        opt.TOTAL_PASS++;
        opt.TOTAL_TESTS_PASS++;
  ...
  s(label, k = 0) {
    ...
    if (typeof opt._FUNCTIONS !== "object" || opt._FUNCTIONS === null || Array.isArray(opt._FUNCTIONS)) {
      opt._FUNCTIONS = /* @__PURE__ */ Object.create(null);
    }
  ...
  /** @returns {string} */
  hexR() {
    return (Math.random() * 1048575 * 1e6).toString(16);
  },
  ...
  gNull(n = 1) {
    ...
    const a = [];
    for (let i = 0; i < n; i++)
      a.push(nulls[Math.random() * nulls.length | 0]);
    return a;
  },
